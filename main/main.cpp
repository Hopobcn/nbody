                      ...
                      float milliseconds, int iterations)
{
    ...
    interactionsPerSecond = (float)numBodies * (float)numBodies * 1e-9 * iterations * 1000;
    interactionsPerSecond /= milliseconds;
    gflops = interactionsPerSecond * (float)flopsPerInteraction;

    printf("---numBodies %d iterations %d millisecons %f interactionsPerSecond %lf gflops %lf\n",
            numBodies, iterations, milliseconds, interactionsPerSecond, gflops);
}
...
void checkGLErrors(const char *s)
{
    ...
    {
        fprintf(stderr, "%s: error - %s\n", s, "gluErrorString no disponible"/*(char *) gluErrorString(error)*/);
    }
}
...
void reshape(int w, int h)
{
    ...
    glLoadIdentity();
    perspectiveGL(60.0, (float) w / (float) h, 0.1, 1000.0);
