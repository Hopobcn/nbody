...
template <typename T>
BodySystemCPU<T>::BodySystemCPU(unsigned numBodies)
...
template <typename T>
void BodySystemCPU<T>::_initialize(unsigned numBodies)
{
...
template <typename T>
void BodySystemCPU<T>::loadFile(const std::string &filename)
{
    if (m_bInitialized)
        _finalize();
